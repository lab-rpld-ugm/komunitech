...
from app.models import Pengguna, Kategori, Project, Kebutuhan, Komentar, Dukungan, Media
...
def save_file(file, destination_type, id=None):
    if file and allowed_file(file.filename):
        ...
        # Buat nama file unik
        unique_filename = f"{destination_type}_{id}_{filename}" if id else f"{destination_type}_{filename}"
...
def daftar_project():
    ...
    next_url = url_for('daftar_project', halaman=projects.next_num) \
        if projects.has_next else None
    prev_url = url_for('daftar_project', halaman=projects.prev_num) \
        if projects.has_prev else None
    return render_template('daftar_project.html', title='Daftar Project',
                           projects=projects.items, next_url=next_url, prev_url=prev_url)
...
def detail_project(id):
    ...
    kebutuhan = Kebutuhan.query.filter_by(project_id=project.id).order_by(Kebutuhan.timestamp.desc()).all()
    return render_template('detail_project.html', title=project.judul,
                           project=project, kebutuhan=kebutuhan)
...
def ajukan(project_id):
    ...
    return render_template('ajukan.html', title='Ajukan Kebutuhan', form=form, project=project)
...
def detail_kebutuhan(project_id, id):
    ...
    return render_template('detail.html', title=kebutuhan.judul,
                           project=project, kebutuhan=kebutuhan, komentar=komentar, form=form,
...
def dukungan(project_id, id):
    ...
    # Tambahkan dukungan
    dukungan = Dukungan(
        pendukung=current_user,
        kebutuhan_didukung=kebutuhan
    )
...
def dukungan_saya():
    ...
    next_url = url_for('dukungan_saya', halaman=dukungan_paginate.next_num) \
        if dukungan_paginate.has_next else None
    prev_url = url_for('dukungan_saya', halaman=dukungan_paginate.prev_num) \
        if dukungan_paginate.has_prev else None
        
    return render_template('dukungan_saya.html', title='Dukungan Saya',
                           dukungan=dukungan_paginate.items, next_url=next_url, prev_url=prev_url)
...
def kebutuhan_saya():
    ...
    kebutuhan = Kebutuhan.query.filter_by(pengguna_id=current_user.id).order_by(
        Kebutuhan.timestamp.desc()).paginate(
        page=halaman, per_page=per_halaman, error_out=False)
    
    next_url = url_for('kebutuhan_saya', halaman=kebutuhan.next_num) \
        if kebutuhan.has_next else None
    prev_url = url_for('kebutuhan_saya', halaman=kebutuhan.prev_num) \
        if kebutuhan.has_prev else None
        
    return render_template('kebutuhan_saya.html', title='Kebutuhan Saya',
...
def project_saya():
    ...
    projects = Project.query.filter_by(pengguna_id=current_user.id).order_by(
        Project.timestamp.desc()).paginate(
        page=halaman, per_page=per_halaman, error_out=False)
    
    next_url = url_for('project_saya', halaman=projects.next_num) \
        if projects.has_next else None
    prev_url = url_for('project_saya', halaman=projects.prev_num) \
        if projects.has_prev else None
        
    return render_template('project_saya.html', title='Project Saya',
                           projects=projects.items, next_url=next_url, prev_url=prev_url)
