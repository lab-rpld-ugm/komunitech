...
class ProjectForm(FlaskForm):
    judul = StringField("Judul Project", validators=[
        DataRequired(message="Judul project harus diisi"), 
        Length(min=5, max=140)
    ])
    kategori = SelectField("Kategori", coerce=int, validators=[
        DataRequired(message="Kategori harus dipilih")
    ])
    deskripsi = TextAreaField("Deskripsi Project", validators=[
        DataRequired(message="Deskripsi project harus diisi"), 
        Length(min=10, max=5000)
    ])
    gambar = FileField("Gambar Project (Opsional)", validators=[
        FileAllowed(["jpg", "png", "jpeg", "gif"], "Hanya file gambar yang diperbolehkan")
    ])
...
class KebutuhanForm(FlaskForm):
    judul = StringField("Judul Kebutuhan", validators=[
        DataRequired(message="Judul kebutuhan harus diisi"), 
        Length(min=5, max=140)
    ])
    kategori = SelectField("Kategori", coerce=int, validators=[
        DataRequired(message="Kategori harus dipilih")
    ])
    deskripsi = TextAreaField("Deskripsi Kebutuhan", validators=[
        DataRequired(message="Deskripsi kebutuhan harus diisi"), 
        Length(min=10, max=5000)
    ])
    prioritas = SelectField("Prioritas", choices=[
        ("Rendah", "Rendah"), 
        ("Sedang", "Sedang"), 
        ("Tinggi", "Tinggi")
    ], validators=[DataRequired()])
    gambar = FileField("Gambar Kebutuhan (Opsional)", validators=[
        FileAllowed(["jpg", "png", "jpeg", "gif"], "Hanya file gambar yang diperbolehkan")
    ])
    delete_gambar = BooleanField("Hapus Gambar")
...
class KomentarForm(FlaskForm):
    isi = TextAreaField("Komentar", validators=[
        DataRequired(message="Komentar tidak boleh kosong"), 
        Length(min=2, max=1000)
    ])
    gambar = FileField("Tambahkan Gambar (Opsional)", validators=[
        FileAllowed(["jpg", "png", "jpeg", "gif"], "Hanya file gambar yang diperbolehkan")
    ])
...
class KategoriForm(FlaskForm):
    nama = StringField("Nama", validators=[
        DataRequired(message="Nama kategori harus diisi"), 
        Length(min=2, max=64)
    ])
    deskripsi = TextAreaField("Deskripsi", validators=[
        DataRequired(message="Deskripsi kategori harus diisi"), 
...
class UserProfileForm(FlaskForm):
    ...
    bio = TextAreaField("Bio", validators=[
        Optional(),
        Length(max=500, message="Bio maksimal 500 karakter")
    ])
...
class ChangePasswordForm(FlaskForm):
    old_password = PasswordField("Password Lama", validators=[
        DataRequired(message="Password lama harus diisi")
    ])
...
class SearchForm(FlaskForm):
    query = StringField("Cari", validators=[
    ...
    search_type = SelectField("Tipe", choices=[
        ("all", "Semua"),
        ("projects", "Project"),
        ("kebutuhan", "Kebutuhan")
    ])
...
class StatusUpdateForm(FlaskForm):
    ...
    reason = TextAreaField("Alasan (Opsional)", validators=[
        Optional(),
        Length(max=500)
    ])
...
class AdminUserForm(FlaskForm):
    ...
    new_password = PasswordField("Password Baru (kosongkan jika tidak diubah)", validators=[
        Optional(),
        Length(min=6, message="Password minimal 6 karakter")
    ])
